 ...
 */
router.post("/draft/:clientId", authenticateToken, async (req: AuthRequest, res: any) => {
  try {
 ...
 */
router.patch("/:id/finalize", authenticateToken, async (req: AuthRequest, res: any) => {
  try {
    const { id } = req.params;
 ...
 */
router.patch("/:id/sign", authenticateToken, async (req: AuthRequest, res: any) => {
  try {
    const { id } = req.params;
...
router.get("/:id/pdf", authenticateToken, async (req: AuthRequest, res: any) => {
  try {
    const id = Number(req.params.id);
    if (!id || isNaN(id)) return res.status(400).json({ error: "Contract id must be a number" });
 ...
 */
router.post("/full/:clientId", authenticateToken, async (req: AuthRequest, res: any) => {
  try {
    const { clientId } = req.params;
    const { unitId, monthlyRate, startDate, endDate } = req.body;
    const createdBy = req.user.id;
    ...
    const fullContract = await prisma.contract.create({
      data: {
        clientId,
        unitId: Number(unitId),
        monthlyRate,
