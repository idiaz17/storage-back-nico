...
router.post("/", async (req, res) => {
    try {
        ...
        const { type, status, clientId, monthlyRate, latitude, longitude, address_street, city, province, country, postal_code, } = req.body;
        const newUnit = await prisma_1.default.unit.create({
            data: {
                type,
                status,
...
router.put("/:id", async (req, res) => {
    ...
    try {
        const { type, status, clientId, monthlyRate, latitude, longitude, address_street, city, province, country, postal_code, } = req.body;
        const updatedUnit = await prisma_1.default.unit.update({
            where: { id },
            data: {
                type,
                status,
...
router.patch("/:id/assign", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const unit = await prisma_1.default.unit.findUnique({ where: { id: unitId } });
        if (!unit)
            return res.status(404).json({ error: "Unit not found" });
